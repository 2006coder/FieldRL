...
def main():
    ...
    parser.add_argument("--max_depth", type=float, default=6, help="Depth truncation in meters.")
    ...
    parser.add_argument("--sample_stride", type=int, default=6,
    ...
    T_c2w_list = []  # We'll store the camera-to-world matrices we actually used
    points2d_per_image = [[] for _ in range(n)]
    points3D = []
    next_pid = 1
    ...
    for i in range(n):
        ...
        T_c2b = np.load(pose_files[i])  # Provided as camera->base
        if T_c2b.shape != (4, 4):
            raise ValueError(f"Pose {pose_files[i]} is not 4x4")

        rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(
            color_raw, depth_raw, depth_scale=args.depth_scale, depth_trunc=max_depth, convert_rgb_to_intensity=False
        )

        pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            rgbd_image,
            intr,
            extrinsic=np.linalg.inv(T_c2b)
        )
        scene += pcd
        ...
        # Create 3D points and tracks from depth for this image
        ...
        for y in range(0, h, stride):
            for x in range(0, w, stride):
                ...
                Xw = R_cw @ Xc + t_cw
                rgb = color_img[y, x]
                # Track index is current length of this image's 2D list
                p2d_idx = len(points2d_per_image[i])
                pid = next_pid
                next_pid += 1
                points3D.append({
                    "id": pid,
                    "xyz": (float(Xw[0]), float(Xw[1]), float(Xw[2])),
                    "rgb": (int(rgb[0]), int(rgb[1]), int(rgb[2])),
                    "error": 0.0,
                    "track": [(i + 1, p2d_idx)],  # image ids start at 1
                })
                points2d_per_image[i].append((float(x), float(y), pid))
