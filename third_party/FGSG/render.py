...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, args):
    with torch.no_grad():
        ...
        if args.video:
            # Use test cameras if available, otherwise use training cameras
            camera_views = scene.getTestCameras()
            if len(camera_views) == 0:
                print("No test cameras found, using training cameras for video rendering")
                camera_views = scene.getTrainCameras()
                
            render_video(dataset.source_path, dataset.model_path, scene.loaded_iter, 
                         camera_views, gaussians, pipeline, background, args.fps)
